...
public class OldLogin : LobbyModule<OldLogin> {
	...
	void Start() {
		...
		clientVersionNumber = Version.instance.clientVersionNumber;
	...
	public override void Draw() {
		...
		switch(GameManager.currentState) {
			...
			case State.PleaseUpdateClient:	PleaseUpdateGUI();		break;
...
");
	...
	// Draw change log
	void DrawChangeLog() {
		if(GameManager.inLogIn && !AccountManager.isLoggedIn && !string.IsNullOrEmpty(OldLogin.changeLog)) {
			using(new GUIArea(changeLogRect)) {
				using(new GUIVertical("box")) {
					using(new GUIScrollView(ref changeLogScroll)) {
						GUILayout.Label(OldLogin.changeLog);
					}
				}
			}
		}
	...
	public bool canLogIn {
		get {
			...
			return (
				GameDB.IsTestAccount(accountEmail) || 
				(
					OldLogin.instance.loginRequestCount == OldLogin.instance.loginRequestResponseCount &&
					Validator.email.IsMatch(accountEmail)
				)
			);
		}
...
#region RPCs
	...
	void VersionNumber(int nServerVersionNumber) {
		...
		if(clientVersionNumber >= serverVersionNumber) {
			...
			ChangeState(State.PleaseUpdateClient);
		}
	}
#endregion
}